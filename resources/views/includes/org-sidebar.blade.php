...
else{ $avatar = url('storage/tenant_'.tenant()->id.auth()->user()->avatar); }
$sidebar = orgsidebarMenu();
@endphp
<aside class="app-sidebar">
    <div class="app-sidebar__logo">
        <a class="header-brand" href="{{url('/')}}">
            <img src="{{ tenantAsset().'admin/assets/images/logo.png' }}" class="header-brand-img desktop-lgo"
                alt="Neobench logo">
        </a>
    </div>
    <div class="app-sidebar__user">
        <div class="dropdown user-pro-body text-center">
            ...
            <div class="user-info">
                <h5 class=" mb-1">{{auth()->user()->fname.' '.auth()->user()->lname}} <i
                        class="ion-checkmark-circled  text-success fs-12"></i></h5>
                <span class="text-muted app-sidebar__user-name text-sm">{{OrgroleData()->role}}</span><br>
    ...
    <ul class="side-menu app-sidebar3">
        ...
        <li class="slide {{ $pr_class }}">
            <?php $menulink = url($pt['link']);
                                ?>
            <a class="side-menu__item {{ $pt['class'] }} " @if($child && count($child)> 0) data-toggle="slide" @endif
                href="{{$menulink}}">
                @if($pt['menu_icon'] !="")
            ...
            <ul class="slide-menu">
                ...
                <?php $menu_link = $ch['link'];
                                ?>
                <li class='<?php echo activeMenu($menu_link); ?> {{ $ch_class }}'><a
                        href='<?php echo url("$menu_link") ?>' class="slide-item">{{$ch['module_name']}}</a></li>
                @endforeach
            </ul>
            @else
            </a>
