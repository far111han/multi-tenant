@php
    ...
    $avatar = tenantAsset().'admin/assets/images/admin_default.png';
    }
    else{
    $avatar = url('storage/tenant_'.tenant()->id.auth()->user()->avatar);
    }
@endphp
<!--app header-->
<style>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const defaultImageSrc = "{{ asset('admin/assets/images/noimage.png') }}";
    let errorCount = 0;

    function handleImageError() {
      errorCount++;
      if (errorCount === images.length) {
        replaceAllImagesWithDefault();
      }
    }

    function replaceAllImagesWithDefault() {
      for (const image of images) {
        image.removeEventListener("error", handleImageError);
        if (image.naturalWidth === 0) {
          // Image has an error (not loaded or failed to load)
    ...
    for (const image of images) {
      ...
      // Check if the image has already been loaded (cached)
      if (image.complete) {
        if (image.naturalWidth === 0) {
          // Image has an error (not loaded or failed to load)
          image.src = defaultImageSrc;
									...
									<div class="d-flex order-lg-2 ml-auto">
										...
										<div class="dropdown header-notify">
											<a class="nav-link icon" data-toggle="dropdown">
												...
												<span class="badge badge-success side-badge" id="unread"></span>
											</a>
											<div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow  animated">
												<div class="dropdown-header">
												...
												<div class="notify-menu">
													<div id="message-list"style="font-size:12px;padding: 10px;display: inline-block;">
													</div>
												</div>
												<div class=" text-center p-2 border-top">
													<a href="{{url('organization/notifications/page')}}" class="">View All Notifications</a>
												</div>
											</div>
										</div>
										<div class="dropdown profile-dropdown">
											...
											<div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow animated">
												<div class="text-center">
													...
													<span class="text-center user-semi-title">{{OrgroleData()->usr_role_name}}</span>
													<div class="dropdown-divider"></div>
												</div>
                                                @if(OrgroleData()->role_type === 'admin')
                                                    <a class="dropdown-item d-flex" href="{{url('organization/admin/profile')}}">
                                                ...
                                                @else
                                                    <a class="dropdown-item d-flex" href="{{url('organization/user/profile')}}">
                                                        <svg class="header-icon mr-3" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zM7.07 18.28c.43-.9 3.05-1.78 4.93-1.78s4.51.88 4.93 1.78C15.57 19.36 13.86 20 12 20s-3.57-.64-4.93-1.72zm11.29-1.45c-1.43-1.74-4.9-2.33-6.36-2.33s-4.93.59-6.36 2.33C4.62 15.49 4 13.82 4 12c0-4.41 3.59-8 8-8s8 3.59 8 8c0 1.82-.62 3.49-1.64 4.83zM12 6c-1.94 0-3.5 1.56-3.5 3.5S10.06 13 12 13s3.5-1.56 3.5-3.5S13.94 6 12 6zm0 5c-.83 0-1.5-.67-1.5-1.5S11.17 8 12 8s1.5.67 1.5 1.5S12.83 11 12 11z"/></svg>
                                                        <div class="">Profile</div>
                                                    </a>
                                                @endif
												<a class="dropdown-item d-flex" href="{{url('/organization/logout')}}">
